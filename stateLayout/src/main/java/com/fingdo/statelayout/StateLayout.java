...
public class StateLayout extends FrameLayout {
     ...
     * 展示空数据的界面
     */
    public void showEmptyView(String tips, String retry) {
        setTipText(EMPTY, tips + "", retry + "");
        AnimationHelper.switchViewByAnim(useAnimation, viewAnimProvider, currentShowingView, emptyView);
        currentShowingView = emptyView;
        currentState=EMPTY;
    }
     ...
     * @param text 文字
     */
    public void setTipText(int type, String text, String retry) {
        switch (type) {
            case ERROR:
                ((ErrorViewHolder) errorView.getTag()).tvTip.setText(text);
                break;
            case EMPTY:
                ...
                if (!TextUtils.isEmpty(retry)) {
                    ((EmptyViewHolder) emptyView.getTag()).tvRetry.setText(retry);
                }
                break;
            case TIMEOUT:
                ((TimeOutViewHolder) timeOutView.getTag()).tvTip.setText(text);
